...
namespace HoursAccountability
{
    ...
    {
        internal readonly static TwinfieldSession.Session TwfSession = new HoursAccountability.TwinfieldSession.Session();
        ...
        private void lvHours_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!_currentProject.Time.Running && lvHours.SelectedItems.Count == 1)
            {
                ...
                findProject.Code = item.SubItems[2].Text;
                findActivity.Code = item.SubItems[3].Text;
        ...
        void findProject_CodeChanged(object sender, EventArgs e)
        {
            findActivity.SetFinderParam("prj1", findProject.Code);
            findActivity.PopulateAutoComplete();
        }
        ...
        {
            findProject.SetFinderParam("trscode", lstType.Text);
            findProject.PopulateAutoComplete();

            findActivity.SetFinderParam("trscode", lstType.Text);
        }
    }
}
